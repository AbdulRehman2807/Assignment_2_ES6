...
let option = document.getElementsByClassName("xyz");
...
const arraykeys = [];
array.map((key) => {
  Object.keys(key).map((nastedKey) => {
    if (!arraykeys.includes(nastedKey)) {
      arraykeys.push(nastedKey);
    }
  });
});
// drop.innerHTML += `<option disable  '></option>`;
arraykeys.map((i) => {
  drop.innerHTML += `<option class="xyz" value="${i}" '>${i}</option>`;
});

var key ;
const slected = (e) => {
  // e.options.selectedIndex;;
  key = option[e.options.selectedIndex].value;
};
console.log(key);
// const optionValue = option[key.options.selectedIndex].value;

const result = () => {
  const filterValue = array.filter((x) => {
    if (x.key == search.value) {
      console.log();
      return x;
    }
  });

  filterValue.length == 0 ? alert("not found") : null;
